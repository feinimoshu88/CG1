...
class TestTouziyyy1(unittest.TestCase):
    ...
    def test_register(self):
        ...
        for u in registers:
            with self.subTest (register=u):
                driver.find_element_by_id("login").send_keys(int(u['tel']))
        time.sleep(6)
        # 手动输入问题
        ...
        cursor.execute ("SELECT message_note from sys_tel_message where tel=%s", (u['tel'],))
        # 提交SQL
        connection.commit ()
        t = cursor.fetchall ()
        # a=t['tel']
